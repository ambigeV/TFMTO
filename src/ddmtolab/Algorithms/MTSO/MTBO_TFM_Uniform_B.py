"""
MTBO-TFM-Uni-B: MTBO-TFM-Uni variant that selects candidates by predicted mean only.

Identical to MTBO-TFM-Uni except the acquisition criterion is:
...
Budget and all other hyperparameters are identical to MTBO-TFM-Uni.
"""
import time
import warnings
import functools
...
class MTBO_TFM_Uniform_B:
    """
    MTBO-TFM-Uni-B: uniform multi-task TabPFN with mean-only acquisition.

    All tasks are pooled into a single training set (with task-ID feature).
            ...
            for i in active_tasks:
                if self.acq_optimizer == 'cmaes':
                    def _score(cands, _Xa=X_all, _ya=y_all, _i=i):
                        ...
                        m = tabpfn_predict(_Xa, _ya, X_test, return_std=False,
