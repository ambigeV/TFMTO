    ...
    def optimize(self):
        ...
        while sum(nfes_per_task) < sum(max_nfes_per_task):
            ...
            for i in active_tasks:
                ...
                X_holdout_enc = self._encode_np(pad_to_dim(X_holdout, max_dim), i, nt)
                X_train_i_enc = self._encode_np(pad_to_dim(decs[i], max_dim), i, nt)

                X_distill = np.vstack([X_lhs_enc, X_train_i_enc])
                X_query   = np.vstack([X_distill, X_holdout_enc])
                ...
                X_ho_t = torch.tensor(
                    X_holdout_enc, dtype=torch.float32, device=device
                )
                with torch.no_grad():
