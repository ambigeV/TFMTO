"""
Demo: MAP-Asym (logEI + LCB) on SepArmMTSO P1–P9

Sequential single-process run — use this to surface runtime errors
that get swallowed by the parallel workers in batched_demo_test.py.
"""
import sys
import os
...
PROBLEMS = {
    'P1': benchmark.P1, 'P2': benchmark.P2, 'P3': benchmark.P3,
    'P4': benchmark.P4, 'P5': benchmark.P5, 'P6': benchmark.P6,
    'P7': benchmark.P7, 'P8': benchmark.P8, 'P9': benchmark.P9,
}
...
for prob_name, prob_fn in PROBLEMS.items():
    print(f"\n{'='*60}")
    print(f"Problem: SepArmMTSO-{prob_name}")
    ...
    for run_id in range(1, N_RUNS + 1):
        ...
        def data_path(algo_name):
            path = f'./Data_SepArmMTSO/{prob_name}/{algo_name}'
        ...
        MTBO_TFM_MAP_Asym(problem, n_initial=N_INITIAL, max_nfes=MAX_NFES,
                          ...
                          lbfgs_iter=MAP_LBFGS_ITER,
                          save_path=data_path(f'MAP-Asym-{MAP_LAMBDA_0}-{MAP_LAMBDA_DECAY}-logEI'),
                          name=run_name(f'MAP-Asym-{MAP_LAMBDA_0}-{MAP_LAMBDA_DECAY}-logEI'),
                          disable_tqdm=False).optimize()
        ...
        MTBO_TFM_MAP_Asym(problem, n_initial=N_INITIAL, max_nfes=MAX_NFES,
                          ...
                          lbfgs_iter=MAP_LBFGS_ITER,
                          save_path=data_path(f'MAP-Asym-{MAP_LAMBDA_0}-{MAP_LAMBDA_DECAY}-LCB'),
                          name=run_name(f'MAP-Asym-{MAP_LAMBDA_0}-{MAP_LAMBDA_DECAY}-LCB'),
                          disable_tqdm=False).optimize()
...
for prob_name in PROBLEMS:
    print(f"\n{'='*60}")
    print(f"Analyzing: SepArmMTSO-{prob_name}")
    # print(f"Analyzing: CEC17-MTSO-50D-{prob_name}")
    print(f"{'='*60}")

    analyzer = DataAnalyzer(
        data_path=f'./Data_SepArmMTSO/{prob_name}',
        save_path=f'./Results_SepArmMTSO/{prob_name}',
